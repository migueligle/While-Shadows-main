...
export default {
  ...
  methods: {
    ...
    fetchData (page = 1) {
      ...
      if (this.$route.query.nameSearch !== undefined) {
        params.name = this.$route.query.nameSearch
      }
      fetchProductsBase(params)
        .then((response) => {
  ...
  watch: {
    '$route.query.nameSearch': function (newNameSearch, oldNameSearch) {
      this.fetchData()
    }
  }
